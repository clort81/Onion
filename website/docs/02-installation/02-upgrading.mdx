...
2. Copy the 7 folders to your SD card, using the following command (replace `/run/media/mmcblk0p1` with the path to your SD card):
   ...
   ```
   However NB: The wildcard Onion-v4.21.0/* will NOT copy .tmp_update folder over to your card.  The unix/bsd/linux "cp" command will not match files or directories beginning with a dot to the wildcard character.
